...
\begin{Exercise}[number={80}]
    \begin{center}\begin{tikzpicture}[scale=0.8, domain=-7:7, smooth, samples=100]
        \draw[gray,very thin] (-7,-3) grid (7,4);
        \draw[thick, -{Latex[length=3mm]}] (-7,0) -- (7,0) node[anchor=north east, scale=0.75] {$x$};
        \draw[thick, -{Latex[length=3mm]}] (0,-3) -- (0,4) node[anchor=north east, scale=0.75] {$y$};
        ...
        \draw[line width=0.5mm, -{Latex[length=2mm]}] (0,0) -- (0,1) node[anchor=north east, scale=0.7] {$\vec{\jmath}$};
        \draw[line width=0.5mm, -{Latex[length=2mm]}] (0,0) -- (1,0) node[anchor=north east, scale=0.7] {$\vec{\imath}$};
        \draw (0,0) node[anchor=north east, scale=0.7] {O};
        
        \begin{axis}[anchor=origin,x=1cm, y=1cm,hide axis,restrict y to domain=-3:4]
